import { qisLogo, header, footer, icon, presentSteps } from './shared.js';
import './slide-08.css';
...
export const slide08 = {
  id: 'slide-08',
  render: () => `
    ${header(14, 'L’essentiel', 'Votre information QdS. <em>À portée de main.</em>')}
    <div class="recap-intro"><img class="recap-qis" src="${qisLogo}" alt="QIS — Qualité Infos Services" /><h2>Un point de repère.<br />Quatre réflexes.</h2><p>Une plateforme commune pour retrouver l’information utile à mes activités.</p><div class="recap-symbol" aria-hidden="true">${icon('branch')}</div></div>
    <ol class="recap-actions"><li><span>01</span><div><h2>Consulter</h2><p>Les informations des périmètres auxquels je suis abonné.</p></div>${icon('branch')}</li><li><span>02</span><div><h2>Suivre</h2><p>Les communications et l’évolution des incidents.</p></div>${icon('mail')}</li><li><span>03</span><div><h2>Vérifier</h2><p>La situation quotidienne des TP et des batchs.</p></div>${icon('cloud-check')}</li><li><span>04</span><div><h2>Choisir</h2><p>Mes notifications Mail, séparément pour Communications et Météo.</p></div>${icon('bell')}</li></ol>
