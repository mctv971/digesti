...
export const slide06 = {
  id: 'slide-06',
  render: () => `
    ${header(6, 'Démonstration · abonnements & notifications', 'Mes abonnements. <em>Mes choix de notification.</em>')}
  ...
  enter(context) {
    ...
    el.querySelector('.reset-preferences').addEventListener('click', () => { form.reset(); status.textContent = ''; updateExample(); }, { signal: events.signal });

    const disposeSteps = presentSteps(context, ['Modifier mes notifications', 'Choisir mes notifications', 'Consulter la Météo'], async ({ step, animate, cursor }) => {
      if (step === 1 && animate && !await cursor(1134, 25, true)) return;
      if (step === 2 && animate) {
        const viewport = el.querySelector('.screen-viewport');
        const area = viewport.getBoundingClientRect();
        const target = weather.getBoundingClientRect();
      ...
      weather.value = step === 2 ? 'Aucune notification' : 'Mail';
      el.querySelector('.demo-cursor').style.opacity = '0';
      el.querySelector('.demo-number').textContent = step === 0 ? '01 / MES ABONNEMENTS' : step === 1 ? '02 / DEUX CANAUX' : '03 / MON CHOIX';
      el.querySelector('.demo-copy h2').innerHTML = step === 0 ? 'Mes abonnements.<br />Mes notifications.' : step === 1 ? 'Deux réglages<br />indépendants.' : 'Mes notifications.<br />Mon choix.';
      el.querySelector('.demo-copy > p').textContent = step === 0 ? 'Je consulte mes périmètres, puis je modifie leurs canaux de notification directement depuis cet écran.' : step === 1 ? 'Pour un SI, je règle séparément les notifications Communications et les notifications Météo.' : 'Je peux recevoir les Communications par Mail et consulter la Météo dans QIS, sans notification Mail.';
      if (step < 2) el.querySelector('.demo-note').innerHTML = '<strong>Pour chaque abonnement SI</strong><br />Communications : Mail ou aucune notification.<br />Météo : Mail ou aucune notification.';
      else updateExample();
    });
